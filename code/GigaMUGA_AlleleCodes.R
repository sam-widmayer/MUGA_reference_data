...
library(tidyr)
################################################################################
# Script to prepare qtl2-style GigaMUGA consensus files from 
# GigaMUGA QC'd genotypes.
#
#
# Sam Widmayer
...
writeFounderGenos <- function(founder_geno, founder_geno_chr){
  ...
  qtl2convert::write2csv(df = recodedFounderGeno,
                         filename = paste0("output/GigaMUGA/GM_foundergeno",
                                           founder_geno_chr,".csv"), 
                         comment = paste0("Founder genotypes (A/B/-) for GigaMUGA chromosome ", 
                                          founder_geno_chr),
                         overwrite = T)
...
gm_ann <- vroom::vroom("data/GigaMUGA/gm_uwisc_v4.csv")
...
write.csv(allele_codes, 
          file = "output/GigaMUGA/GM_allele_codes_GRCm39.csv", 
          quote = F, row.names = F)
