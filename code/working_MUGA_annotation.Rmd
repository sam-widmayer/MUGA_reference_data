...
founder_background_QC <- list()
for(f in founders){
  print(paste0("Beginning QC: ", f))
  maternalF1s <- all_founder_samples_parents %>%
    dplyr::filter(dam == f,
                  bg == "F1") %>%
    dplyr::mutate(sire = as.factor(sire))
  sires <- levels(maternalF1s$sire)
  

  # Import focal founder
  mom <- get(ls(pattern = paste0("Calls_",f)))
  mom <- mom[which(mom[,2]!="N"),]
  
  geno_match_results <- list()
  for(s in sires){
    print(paste0("Calculating concordance: (",f,"x",s,")F1"))
    dad <- get(ls(pattern = paste0("Calls_",s)))
    dad <- dad[which(dad[,2]!="N"),]
    
    # Generate theoretical F1 genotype with consensus genotypes from both parents
    ...
    # Find samples for that F1 background
    sample_genos <- maternalF1s %>%
      dplyr::filter(sire == s) %>%
      # Attach genotypes
      dplyr::right_join(genos.flagged,.,by = "sample") %>%
      # Remove markers without clear calls or that were not present in founder strain's consensus genotype object
      dplyr::filter(marker %in% mom$marker, 
                    genotype != "N") %>%
      dplyr::distinct(marker, sample, genotype, .keep_all = TRUE) %>%
      # Remove markers flagged for high missingness
      dplyr::filter(marker_flag != "FLAG")
  
    # If there was more than one sample for a given F1 background, we checked to see if any markers produced different genotypes (i.e. one sample was H but other was not)
    ...
    # Remove markers with disagreements between samples for the same genetic background 
    geno_comp <- sample_genos %>%
      dplyr::filter(!marker %in% marker_disagreements$marker) %>%
      # Attach the predicted cross genotypes
      dplyr::inner_join(.,cross %>%
                          dplyr::ungroup() %>%
                          dplyr::select(marker,predicted_genotypes)) %>%
      # Determine whether each sample genotype matches that predicted
    ...
    # Count the number of marker matches and mismatches
    geno_comp_summary <- geno_comp %>%
      dplyr::group_by(sample, inferred.sex, alt_chr, matching_genos) %>%
      dplyr::count() %>%
      tidyr::pivot_wider(names_from = matching_genos, values_from = n) %>%
      dplyr::mutate(dam = f, sire = s)
  
  geno_match_results[[s]] <- list(geno_comp, geno_comp_summary)
  }
  geno_concordance <- purrr::transpose(geno_match_results)[[1]]
  geno_comp_summaries <- purrr::transpose(geno_match_results)[[2]]
  
  founder_background_QC[[f]] <- list(geno_concordance,
                                     geno_comp_summaries)
}
founder_background_QC_tr <- purrr::transpose(founder_background_QC)
founder_concordance_df <- purrr::map(.x = founder_background_QC_tr[[2]],
...
names(founder_palette) <- names(founder_background_QC)
ggplot(data = founder_concordance_df_2, mapping = aes(x = sample, 
                                                    y = concordance,
                                                    colour = sire,
                                                    fill = dam)) + 
  ...
  facet_grid(inferred.sex~alt_chr, scales = "free", space = "free") +  
  QCtheme + 
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
```
...
```{r}
founder_background_QC_tr[[1]]$`129S1/SvImJ`$`C57BL/6J` %>%
  dplyr::filter(sample == "(129S1/SvImJxC57BL/6J)F1m15914",
                chr == "X") %>%
  dplyr::select(sample, genotype, predicted_genotypes)
```
