...
gigamuga_read <- function(ch){
  ...
  control_genotypes <- vroom::vroom(file = "../data/GigaMUGA/GigaMUGA_control_genotypes.txt",
                                    num_threads = 32, # trying on sumner
                                    skip = ch,
                                    n_max = n_genos*100,
...
tictoc::tic()
furrr::future_map(control_genotypes_nest_chr$data[1:2], function(x){
  x %>% 
...
control_allele_freqs <- furrr::future_map(control_genotypes_nest_chr,
                  function(x){
