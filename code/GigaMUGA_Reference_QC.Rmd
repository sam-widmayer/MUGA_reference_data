...
writeReferenceGenotypes <- function(founder_file, nonfounder_file){
  if(file.exists(paste0("../data/GigaMUGA/",founder_file))){
    founder_sample_genos <- fst::read.fst(paste0("../data/GigaMUGA/",founder_file)) %>%
      dplyr::filter(!marker %in% above.cutoff$marker) # should be clean, but just in case
    non_founder_sample_genos <- fst::read.fst(paste0("../data/GigaMUGA/",nonfounder_file)) %>%
    ...
    non_founder_sample_genos_int <- fst::read.fst(paste0("../data/GigaMUGA/",nonfounder_file)) %>%
...
writeFounderConsensusGenotypes <- function(founder_file){
  ...
  founder_consensus_genos <- fst::read.fst(paste0("../data/GigaMUGA/",founder_file)) %>%
    dplyr::filter(!marker %in% above.cutoff$marker) # should be clean, but just in case
...
writeReferenceIntensities <- function(nonfounder_file){
    ...
    reference_sample_intensities <- fst::read.fst(paste0("../data/GigaMUGA/",nonfounder_file)) %>%
      dplyr::filter(!sample_id %in% bad_samples$sample_id,
                    !marker %in% above.cutoff$marker) %>%
...
recoded_wide_geno_files <- list.files("../data/GigaMUGA/", pattern = "gm_widegenos_chr")
recoded_wide_sample_genos_list <- list()
for(i in 1:length(recoded_wide_geno_files)){
  recoded_wide_sample_genos_list[[i]] <- fst::read.fst(path = paste0("../data/GigaMUGA/",recoded_wide_geno_files[[i]]))
}
...
chr_concordance_files <- list.files("../data/GigaMUGA/", pattern = "gm_founder_concordance_chr_")
chr_concordance_list <- list()
for(i in 1:length(chr_concordance_files)){
  chr_concordance_list[[i]] <- fst::read.fst(path = paste0("../data/GigaMUGA/",chr_concordance_files[[i]]))
}
...
founder_sample_geno_files <- c(as.list(list.files(path = "../data/GigaMUGA/", pattern = "gm_founder_genos_chr_")[1:19]),
                                  "M",
                                  "NA",
                                  list.files(path = "../data/GigaMUGA/", pattern = "gm_founder_genos_chr_")[20], 
                                  "Y")

nonfounder_sample_geno_files <- as.list(list.files(path = "../data/GigaMUGA/", pattern = "gm_genos_chr_"))
...
#################################################
founder_consensus_files <- list.files(path = "../data/GigaMUGA/", pattern = "gm_founder_consensus_chr_")
