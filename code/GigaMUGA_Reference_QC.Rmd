...
if(detectCores() > length(control_genotype_files)){
  make_chunks(n_x = length(control_genotype_files), n_workers = detectCores())
} else {
  make_chunks(n_x = length(control_genotype_files), chunk_size = detectCores()/8)
}
...
# i.e.
#   marker            allele1     n  freq
#   <chr>             <fct>   <int> <dbl>
# 1 B6_01-011986786-S N           2 0.003
# 2 B6_01-011986786-S C         530 0.92 
# 3 B6_01-011986786-S T          44 0.076
# 4 B6_01-033811444-S N           5 0.009
# 5 B6_01-033811444-S A          50 0.087
# 6 B6_01-033811444-S T         521 0.905
...
```{r Filtering to Chr X and Y Markers}
...
long_XY_intensities <- furrr::future_map(.x = control_genotype_files, 
                                         .f = function(x){
                                          chr_geno <- vroom::vroom(paste0("../data/GigaMUGA/",x),delim = ",")
                                          ints <- chr_geno %>%
                                             dplyr::left_join(., gm_metadata) %>%
                                             dplyr::filter(chr %in% c("X","Y"))
                                          return(ints)}) %>% Reduce(dplyr::bind_rows,.)


# Expected output
#   marker     allele1 allele2 sample_id      X     Y `GC Score` Theta `X Raw` `Y Raw`     R bp_mm10 bp_grcm39 cM_cox strand snp   unique unmapped
#   <chr>      <chr>   <chr>   <chr>      <dbl> <dbl>      <dbl> <dbl>   <dbl>   <dbl> <dbl>   <dbl>     <dbl>  <dbl> <chr>  <chr> <lgl>  <lgl>   
# 1 B6_rs6368… A       A       NIH_TVD_Y… 1.19  0.015      0.824 0.008   10963     220 1.21   5.79e7  56912987  30.7  minus  AG    TRUE   FALSE   
# 2 B6_X-0074… G       G       NIH_TVD_Y… 0.026 0.726      0.884 0.977     441    2848 0.752  7.89e6   7757203   2.23 plus   GA    TRUE   FALSE   
# 3 B6_X-0142… C       C       NIH_TVD_Y… 0.025 0.824      0.925 0.981     563    3415 0.849  1.47e7  14537051   8.28 plus   CT    TRUE   FALSE   
# 4 B6_X-0392… G       G       NIH_TVD_Y… 0.01  0.724      0.906 0.991     299    2841 0.735  4.19e7  40974138  21.7  minus  GA    TRUE   FALSE   
# 5 B6_X-0783… C       C       NIH_TVD_Y… 0.018 0.816      0.930 0.986     376    3190 0.834  8.11e7  80131410  36.8  minus  CA    TRUE   FALSE   
# 6 B6_X-0978… G       G       NIH_TVD_Y… 0.104 1.10       0.788 0.94     1172    4267 1.20   1.01e8  99710707  42.2  minus  GA    TRUE   FALSE   
# # … with 3 more variables: probe <chr>, strand_flipped <lgl>, chr <chr>
