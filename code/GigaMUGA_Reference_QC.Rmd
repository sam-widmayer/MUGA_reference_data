...
## ANNOTATE
gigamuga_read <- function(ch){
  
  print(paste0("Reading genotype data for lines ", ch, "-", c(ch+n_genos*100)))
  control_genotypes_header <- vroom::vroom(file = "../data/GigaMUGA/GigaMUGA_control_genotypes.txt",
                                    num_threads = 1, 
                                    skip = 0,
                                    n_max = 1, col_names = T)
  control_genotypes <- vroom::vroom(file = "../data/GigaMUGA/GigaMUGA_control_genotypes.txt",
                                    num_threads = 16, # trying on sumner
                                    skip = ch,
                                    n_max = n_genos*100,
                                    col_names = colnames(control_genotypes_header))
  colnames(control_genotypes)[1:4] <- c("marker","sample_id", "allele_1", "allele_2")
  
  print(paste0("Attaching marker metatdata for lines ", ch, "-", c(ch+n_genos*100)))
  control_genotypes_meta <- control_genotypes %>%
    dplyr::left_join(., gm_metadata)
  
  
  gm_genotypes <- control_genotypes_meta %>%
    dplyr::select(marker, allele_1, allele_2, sample_id, X, Y)
  print(paste0("Calling genotypes for lines ", ch, "-", c(ch+n_genos*100)))
  gm_genotypes$genotype <- apply(gm_genotypes, 1, callGeno)
...
gm_samples <- vroom::vroom("../data/GigaMUGA/Sample_Map.txt")

## Determining dimensions of data needing to be read in
n_genos <- 143259
n_samples <- nrow(gm_samples)
all_rows <- n_genos*n_samples 
options(scipen = 99999999)
read_chunks <- seq(1, all_rows+1,
                   by = n_genos*100) 


gigamuga_control_genotypes_list <- furrr::future_map(read_chunks,
                                                     gigamuga_read, 
                                                     .progress = T)


# Verify that all markers for all samples were read by future_map file reading
# sample_marker_summary <- furrr::future_map(.x = gigamuga_control_genotypes_list,
#                                            .f =  function(x){
#                                              x %>% 
#                                                dplyr::group_by(sample_id) %>%
#                                                dplyr::count()
#                                              }, .progress = T) %>% Reduce(rbind,.)
...
control_allele_freqs <- furrr::future_map(gigamuga_control_genotypes_list,
                  function(x){
                    control_allele_freqs <- x %>%
...
# 1 AmpR002           N           99  0.99
# 2 AmpR002           T            1  0.01
# 3 B6_01-011986786-S C           74  0.74
# 4 B6_01-011986786-S H           21  0.21
# 5 B6_01-011986786-S T            5  0.05
# 6 B6_01-033811444-S N            1  0.01
...
n.calls.strains <- furrr::future_map(gigamuga_control_genotypes_list,
                                     function(x){
                                       sample_Ns <- x %>%
                                         ...
                                         dplyr::ungroup() %>%
                                         dplyr::group_by(sample_id) %>%
                                         dplyr::mutate(genotype = if_else(condition = genotype == "-", true = "N", false = as.character(genotype)))
                                       return(sample_Ns)}) %>% 
...
#        <int> <chr>          
# 1       8420 129P1/ReJm35858
# 2       7824 C57BL/6CRmep1  
# 3       8188 NIH_TVD_YS-A1  
# 4       8060 NIH_TVD_YS-A10 
# 5       8105 NIH_TVD_YS-A11 
# 6       8762 NIH_TVD_YS-A12 


## Interactive plot of the number of missing genotypes for each sample.
bad_sample_cutoff <- quantile(n.calls.strains$n.no.calls, probs = seq(0,1,0.05))[20]
high.n.samples <- n.calls.strains %>%
  dplyr::filter(n.no.calls > bad_sample_cutoff)
sampleQC <- ggplot(n.calls.strains, 
...
long_XY_intensities <- furrr::future_map(.x = gigamuga_control_genotypes_list, 
                                         .f = function(x){
                                          ints <- x %>%
                                             dplyr::left_join(., gm_metadata) %>%
...
# Expected output
#   marker allele_1 allele_2 sample_id     X     Y genotype chr   bp_mm10 bp_grcm39 cM_cox strand
#   <chr>  <chr>    <chr>    <chr>     <dbl> <dbl> <chr>    <chr>   <dbl>     <dbl>  <dbl> <chr> 
# 1 B6_rs… A        A        NIH_TVD_… 1.19  0.015 A        X      5.79e7  56912987  30.7  minus 
# 2 B6_X-… G        G        NIH_TVD_… 0.026 0.726 G        X      7.89e6   7757203   2.23 plus  
# 3 B6_X-… C        C        NIH_TVD_… 0.025 0.824 C        X      1.47e7  14537051   8.28 plus  
# 4 B6_X-… G        G        NIH_TVD_… 0.01  0.724 G        X      4.19e7  40974138  21.7  minus 
# 5 B6_X-… C        C        NIH_TVD_… 0.018 0.816 C        X      8.11e7  80131410  36.8  minus 
# 6 B6_X-… G        G        NIH_TVD_… 0.104 1.10  G        X      1.01e8  99710707  42.2  minus 
# # … with 5 more variables: snp <chr>, unique <lgl>, unmapped <lgl>, probe <chr>,
# #   strand_flipped <lgl>
...
```{r Preliminary sex prediction, eval=FALSE, include=FALSE}
...
digit.trim <- flagged_XY_intensities %>% 
  # One character
  ...
  dplyr::mutate(predicted.sex = dplyr::case_when(predicted.sex.1 == "m" ~ "m", 
                                                 ...
                                                 TRUE ~ "unknown"))
digit.trim %>%
  dplyr::distinct(sample_id, predicted.sex) %>% View()
...
predicted.sex.table <- predicted.sexes.strings %>%
  ...
  dplyr::select(sample_id, predicted.sex, bg) %>%
...
sex.chr.intensities <- Xchr.int %>%
  ...
  dplyr::mutate(predicted.sex = dplyr::case_when(mean.y.int > 0.2 ~ "m",
                                                 mean.x.chr.int > 0.9 ~ "f",
                                                 TRUE ~ "unknown"))

het_counts <- flagged_XY_intensities %>%
  dplyr::ungroup() %>%
  dplyr::filter(marker_flag == "FLAG") %>%
  dplyr::mutate(genotype = dplyr::if_else(genotype == "-", "N", as.character(genotype))) %>%
  dplyr::filter(genotype != "N") %>%
  dplyr::mutate(alt_genotype = dplyr::if_else(genotype == "H", "het", "no_het")) %>%
  dplyr::left_join(., sex.chr.intensities %>% dplyr::select(sample_id, predicted.sex)) %>%
  dplyr::group_by(sample_id, predicted.sex, chr, alt_genotype) %>%
  dplyr::count() %>%
  tidyr::pivot_wider(names_from = alt_genotype, values_from = n)

het_counts[is.na(het_counts)] <- 0
het_rate_df <- het_counts %>%
  dplyr::mutate(het_rate = het/(het+no_het))
remainder <- het_rate_df %>%
  dplyr::filter(chr == "Y",
                het_rate > 0.3,
                predicted.sex == "unknown")
...
het_rate_df_plot <- ggplot(het_rate_df, mapping = aes(x = predicted.sex, 
                                                      y = het_rate,
                                                      label = sample_id)) + 
  geom_jitter() + 
  facet_grid(.~chr) + 
...
rough_clusters <- kmeans(sex.chr.intensities[,3:4], centers = 2)$cluster
...
dams <- data.frame(tidyr::expand_grid(founder_strains_2, founder_strains_2)) %>%
...
sires <- data.frame(tidyr::expand_grid(founder_strains_2, founder_strains_2)) %>%
  `colnames<-`(c("dams","sires")) %>%
  # filter(dams != sires) %>%
  dplyr::select(sires) %>%
  as.list()
...
concordance_plot <- ggplot(founder_concordance_df_3, mapping = aes(x = dam, y = concordance, color = dam, fill = sire)) + 
  geom_jitter(shape = 21, width = 0.10) +
...
write.csv(final_founder_consensus_genotypes, file = "../output/MegaMUGA/MegaMUGA_founder_consensus_genotypes.csv")
